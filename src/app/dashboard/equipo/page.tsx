...
export default function TeamProfilesPage() {
  ...
  const canManage = hasPermission('teamProfiles.manage');
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [invite, list] = await Promise.all([
        canManage ? authenticatedPost<Invitation>('/api/team-intake/invitation', { action: 'get' }) : Promise.resolve(null),
        authenticatedPost<{ profiles: TeamProfile[]; nextCursor: string | null }>('/api/team-intake/profiles', { action: 'list' }),
      ]);
  ...
  const copyLink = async () => {
    if (!canManage || !invitePath) return;
  ...
  const loadMore = async () => {
    ...
    setBusy('load-more');
    try {
      const page = await authenticatedPost<{ profiles: TeamProfile[]; nextCursor: string | null }>('/api/team-intake/profiles', { action: 'list', cursor: nextCursor });
      setProfiles((current) => [...current, ...page.profiles]);
      setNextCursor(page.nextCursor);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'No se pudo cargar la siguiente página.');
    } finally {
      setBusy('');
    }
  };

  const deleteProfile = async (profile: TeamProfile) => {
    if (!canManage) return;
    if (!window.confirm('Se eliminará esta ficha para permitir que la persona la envíe nuevamente corregida. ¿Deseas continuar?')) return;
    setBusy(profile.id);
    setMessage('');
    try {
      await authenticatedPost('/api/team-intake/profiles', { action: 'delete', id: profile.id });
      setProfiles((current) => current.filter((item) => item.id !== profile.id));
      setSelected(null);
      setMessage('Ficha eliminada. La persona ya puede volver a enviarla con el mismo DNI.');
    } catch (error) {
      setMessage(error instanceof Error ? error.message : 'No se pudo eliminar la ficha.');
    } finally {
      setBusy('');
    }
  };
  ...
  return (
    <div className='mx-auto max-w-[1500px] pb-20'>
      ...
      {canManage && <section className='mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6'>
        <div className='flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between'><div><h2 className='flex items-center gap-2 text-lg font-black text-slate-900'><Link2 size={20} className='text-primary' /> Enlace para WhatsApp</h2><p className='mt-1 text-sm text-slate-500'>No requiere inicio de sesión y solo permite enviar una ficha.</p>{invitation && <p className='mt-1 text-xs font-semibold text-slate-400'>Vence el {new Date(invitation.expiresAt).toLocaleDateString('es-PE')}.</p>}</div>{invitation && <span className={`w-fit rounded-full px-3 py-1.5 text-xs font-bold ${invitation.active ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>{invitation.active ? 'Enlace activo' : 'Enlace cerrado'}</span>}</div>
        ...
        {message && <p role='status' className='mt-3 rounded-xl bg-blue-50 px-4 py-3 text-sm font-bold text-primary-dark'>{message}</p>}
      </section>}
      ...
      {nextCursor && !loading && <div className='mt-4 flex justify-center'><button type='button' onClick={() => void loadMore()} disabled={Boolean(busy)} className='flex min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-5 text-sm font-bold text-slate-700 disabled:opacity-50'>{busy === 'load-more' && <Loader2 size={17} className='animate-spin' />} Cargar más fichas</button></div>}

      {selected && <ProfileDetail profile={selected} busy={busy === selected.id} onClose={() => setSelected(null)} onStatus={(next) => void changeStatus(selected, next)} onDelete={() => void deleteProfile(selected)} />}
    </div>
  );
}

function ProfileDetail({ profile, busy, onClose, onStatus, onDelete }: { profile: TeamProfile; busy: boolean; onClose: () => void; onStatus: (status: TeamProfileStatus) => void; onDelete: () => void }) {
  const { hasPermission } = useAccess();
  const canManage = hasPermission('teamProfiles.manage');
  useEffect(() => {
